classdef DisturbanceLinearSystem < LinearSystem
    ...
    methods (Access = public)
        ...
        function obj = DisturbanceLinearSystem(A, B, Q, R, W, K)
            obj = obj@LinearSystem(A, B, Q, R, K);

            obj.W = W;
            obj.Z = obj.compute_mrpi_set(1e-2);
        end

        function [x_new,w] = propagate(obj, x, u, w)
    ...
    methods (Access = public)
        ...
        function Fs = compute_mrpi_set(obj, epsilon)
            ...
            while(alpha > epsilon/(epsilon + Ms))
                ...
                it = it+1;
                if s>10
                    disp('No feasible minimal robust positively invariant set')
                    fail = true;
                    break
                end
            end
