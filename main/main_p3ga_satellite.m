...
DesignStrategy_array = {'controldesign','CCD'};
for i = 1:2
    DesignStrategy = DesignStrategy_array{i};
%     DesignStrategy = 'CCD';
    if strcmp(DesignStrategy,'CCD')
        SaveLoc_dir = 'results_satellite2\ccd_results';
        xp = [];
    elseif strcmp(DesignStrategy,'plantdesign')
        SaveLoc_dir = 'results_satellite2\plantdesign_results';
        xp = [];
    elseif strcmp(DesignStrategy,'controldesign')
        SaveLoc_dir = 'results_satellite2\controldesign_results';
        xp = [cosd(25),7.5]; % if control design only, the plant design variable is fixed
    elseif strcmp(DesignStrategy,'seqdesign')
        ...
        SaveLoc_dir = 'results_satellite2\seqdesign_results';
        load('results_satellite2\plantdesign_results\plantdesign_results_xp_opt.mat'); % from the plant design optimization results
    end
    % mkdir(SaveLoc_dir);
    ...
    if strcmp(DesignStrategy,'seqdesign')
        ...
        for i = 1:length(xp_opt)
            [K_opt{i},fval_iter{i},M{i}] = runobjconstr_p3ga_satellite(xp_opt(i,:),SaveLoc_dir,DesignStrategy,i);
            x_opt_tmp = [repmat(xp_opt(i),size(K_opt{i},1),1),K_opt{i}];
            x_opt = [x_opt;x_opt_tmp];
            fval = [fval;fval_iter{i}];
        end
    else
        [x_opt,fval,M] = runobjconstr_p3ga_satellite(xp,SaveLoc_dir,DesignStrategy,[]);
    end
    ...
    if strcmp(DesignStrategy,'plantdesign')
        xp_opt = x_opt;
        save(strcat(SaveLoc_dir,'\plantdesign_results_xp_opt.mat'),'xp_opt')
    end
end